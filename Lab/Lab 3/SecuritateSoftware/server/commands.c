...
)
{
    ...
    __try
    {
        ...
        *OutLength = 0;
        ...
        retVal = sscanf_s(Param, "%u", &howMany);
        if (retVal == 0)
        {
            return FALSE;
        }
        ...
        *OutLength += strlen(Output);
        ...
        for (i = 0; i < howMany; i++)
        {
            ...
            if ((*OutLength + strlen(buff)) + 2 > 0 && *OutLength + strlen(buff) + 2 < OutputSize)
            {
                strcat(Output, buff);
...
{
    BOOLEAN ret = TRUE;
    long long quoteIndex = 0;
    int maxQuotes = sizeof(Quotes) / sizeof(Quotes[0]);
    char buff[32] = { 0 };
    ...
    __try
    {
        ...
        *OutLength = 0;
        ...
        if (0 == sscanf_s(Param, "%lld", &quoteIndex) || quoteIndex < 0)
        {
            return FALSE;
        }

        sprintf_s(buff, sizeof(buff), "[Random quote] ");
        if (strlen(buff) < OutputSize)
        {
