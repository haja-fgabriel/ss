...
int __cdecl main(int argc, char **argv) 
{
    ...
    //
    iResult = SetConsoleCP(437);
    if (!iResult)
    {
        printf("Could not set proper encoding for console input characters.\n");
        return 0x100;
    }

    iResult = SetConsoleOutputCP(437);
    if (!iResult)
    {
        printf("Could not set proper encoding for console output characters.\n");
        return 0x100;
    }
    ...
    do {
        ...
        //unsigned char* input = malloc(DEFAULT_BUFLEN);
        
        // Accepting commands
        printf("> ");


        // TODO modify input from client
        ...
        else if (0 == _stricmp(input, "deadbeef"))
        {
            char sendbuf[54] = "pass aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaabeef";
            *(DWORD*)((char*)sendbuf + 49) = 0xdeadbeef;
            iResult = send(ConnectSocket, sendbuf, 54, 0);
        }
        else
        {
